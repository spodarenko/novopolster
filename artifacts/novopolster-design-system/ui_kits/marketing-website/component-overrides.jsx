...
function FAQAccordion({ items = [], defaultOpen = 0 }) {
  ...
  const { isMobile } = window.useViewport();
  return (
    <div style={{ fontFamily: 'var(--font-body)', display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}>
      {items.map((item, i) => {
        const open = openIndex === i;
        return (
          <div key={i} style={{ background: 'var(--color-bg)', borderRadius: 16, padding: isMobile ? '14px 16px' : '16px 24px' }}>
            <button
              ...
              style={{
                ...
                fontSize: isMobile ? 17 : 20, lineHeight: isMobile ? '24px' : '28px', color: 'var(--color-text-primary)',
              }}
            >
              {item.question}
              <span style={{
                position: 'relative', flexShrink: 0, width: isMobile ? 36 : 44, height: isMobile ? 36 : 44, borderRadius: 'var(--radius-pill)',
                background: '#E9F7F4', display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
              }}>
                <span style={{
                  position: 'absolute', top: '50%', left: '50%', width: isMobile ? 16 : 20, height: 2,
                  background: 'var(--color-brand)', borderRadius: 1, transform: 'translate(-50%, -50%)',
                }} />
                <span style={{
                  position: 'absolute', top: '50%', left: '50%', width: isMobile ? 16 : 20, height: 2,
                  background: 'var(--color-brand)', borderRadius: 1,
            ...
            <div style={{
              ...
              <div style={{ overflow: 'hidden', minHeight: 0 }}>
                <p style={{
                  margin: 0, paddingTop: 8, fontSize: 16, lineHeight: '24px', color: 'var(--color-text-primary)',
                }}>
                  {item.answer}
                </p>
              </div>
...
function Button({
  ...
  disabled = false, as = 'button', href, onClick, type = 'button', radius,
}) {
  const base = {
    fontFamily: 'var(--font-display)', fontWeight: 900, textTransform: 'uppercase', letterSpacing: 'var(--tracking-wide)',
    display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
    borderRadius: radius || 'var(--radius-md)', border: '1px solid transparent',
  ...
  const variants = {
    primary: { background: 'var(--color-brand)', color: 'var(--color-text-inverse)', borderColor: 'var(--color-brand)' },
    secondary: { background: 'var(--color-surface-ink)', color: 'var(--color-text-inverse)', borderColor: 'var(--color-surface-ink)' },
    outline: { background: 'transparent', color: 'var(--color-text-primary)', borderColor: 'var(--color-border-strong)' },
    ghost: { background: 'transparent', color: 'var(--color-brand)', borderColor: 'transparent' },
  };
  const hoverBg = {
    primary: 'var(--color-brand-hover)', secondary: 'var(--neutral-800)',
    outline: 'var(--color-bg-sunken)', ghost: 'var(--color-brand-subtle)',
  };
  ...
  const style = {
    ...
    ...(pressed && !disabled ? { background: variant === 'primary' ? '#067A70' : hoverBg[variant] } : hover && !disabled ? { background: hoverBg[variant] } : {}),
    ...(focused && !disabled ? { outline: `2px solid ${variant === 'primary' ? 'var(--color-brand)' : 'var(--color-text-primary)'}`, outlineOffset: 2 } : {}),
  };
  const content = (
    <>
...
function WhatsAppButton({ phone = '491605712229', message = 'Hallo, ich interessiere mich für eine Polsterreinigung.', label = 'Per WhatsApp anfragen', floating = true }) {
  const [hover, setHover] = React.useState(false);
    ...
    return (
      <a
        ...
        style={{
          ...
          width: 60, height: 60, borderRadius: 'var(--radius-pill)', background: 'var(--color-whatsapp)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      >
        <img src={base + "assets/icons/whatsapp.svg"} alt="" style={{ width: 28, height: 28, filter: 'brightness(0) invert(1)' }} />
      </a>
    );
  }
  ...
  return (
    ...
    >
      <img src={base + "assets/icons/whatsapp.svg"} alt="" style={{ width: 20, height: 20, filter: 'brightness(0) invert(1)' }} />
      {label}
    </a>
  );
}
  ...
  return (
    <header style={{
      ...
      <img
        ...
        style={{ height: 18.48, width: 'auto', objectFit: 'contain', cursor: 'pointer' }}
      />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 8 : 4, position: 'relative', flexShrink: 0 }}>
        {!isNarrow ? (
          ...
          >
            <span style={{ width: 18, height: 12, position: 'relative', display: 'block' }}>
              <span style={{ position: 'absolute', left: 0, top: 0, width: 18, height: 2, background: 'var(--color-text-primary)', borderRadius: 2 }} />
              <span style={{ position: 'absolute', left: 0, top: 5, width: 18, height: 2, background: 'var(--color-text-primary)', borderRadius: 2 }} />
...
function LanguageSwitcher({ value = 'DE', onChange, tone = 'light' }) {
  ...
  return (
    <div ref={rootRef} style={{ position: 'relative', fontFamily: 'var(--font-display)' }}>
      <button
        onClick={() => setOpen((o) => !o)}
        ...
        style={{
          ...
          fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: isMobile ? 14 : 'var(--text-base)',
          textTransform: 'uppercase', padding: isMobile ? '8px 10px' : '8px 16px',
          transition: `background var(--duration-base) var(--ease-standard)`,
        }}
      >
      ...
      {open ? (
        <div style={{
          ...
          width: 200, boxSizing: 'border-box', background: '#F7F5F1', border: 0,
          borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,0.1)', overflow: 'hidden', zIndex: 10,
        }}>
          {langs.map((l) => (
            <div
              key={l.code}
...
function ServiceCard({ icon, title, description, priceFrom, onClick }) {
  const [hover, setHover] = React.useState(false);
  return (
    <div
      onClick={onClick}
...
function PriceCard({ row, waHref, ctaLabel = 'Anfragen' }) {
  const [hover, setHover] = React.useState(false);
  ...
  return (
    <div
      ...
      onMouseLeave={() => setHover(false)}
      style={{
        display: 'flex', flexDirection: 'column', gap: isMobile ? 24 : 'var(--space-8)', height: '100%',
        background: 'var(--color-surface)', borderRadius: 24, padding: isMobile ? 24 : 'var(--space-8)',
        boxShadow: hover ? 'var(--shadow-md)' : 'var(--shadow-xs)',
        transform: hover ? 'translateY(-3px)' : 'none',
        transition: `box-shadow var(--duration-base) var(--ease-standard), transform var(--duration-base) var(--ease-standard)`,
      }}
    >
      <div style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: 20, lineHeight: '28px', color: 'var(--color-text-primary)' }}>
        {row.label}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: isMobile ? 30 : 36, lineHeight: isMobile ? '34px' : '40px', textTransform: 'uppercase', color: 'var(--color-text-primary)' }}>
          {price}
        </div>
        {row.desc ? (
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 16, lineHeight: '24px', color: 'var(--color-text-secondary)' }}>
            {row.desc}
          </div>
        ) : null}
      </div>
...
function PricingTabs({ categories = [], defaultTab = 0, ctaLabel = 'Anfragen', phone = '491605712229', message = 'Hallo, ich interessiere mich für eine Polsterreinigung.' }) {
      ...
      {cat ? (
        <React.Fragment>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 'var(--space-5)', alignItems: 'stretch' }}>
            {cat.rows.map((row, i) => (
              <PriceCard key={i} row={row} waHref={waHref} ctaLabel={ctaLabel} />
            ))}
          </div>
          {cat.callout ? (
            <div style={{
...
function Footer({ t }) {
  ...
  const serviceLinks = (t.footer.serviceLinks || t.servicesGrid.items.map((item) => item.title.join(' '))).map((label, index) => ({ label, serviceId: serviceIds[index] }));
  const social = ['instagram', 'facebook', 'tiktok'];
  const heading = { fontFamily: 'var(--font-display)', fontSize: 14, lineHeight: '20px', fontWeight: 700, textTransform: 'uppercase', color: 'rgba(19,20,19,0.64)' };
  const link = { fontFamily: 'var(--font-body)', fontSize: 16, lineHeight: '24px', color: 'var(--color-text-primary)', textDecoration: 'none' };
  const navLink = { fontFamily: 'var(--font-display)', fontSize: 14, lineHeight: '20px', fontWeight: 700, textTransform: 'uppercase', color: 'var(--color-text-primary)', textDecoration: 'none' };
  function FooterLink({ href, children, onClick }) {
    ...
    return (
      <a href={href} onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => { setHover(false); setPressed(false); }} onMouseDown={() => setPressed(true)} onMouseUp={() => setPressed(false)} onFocus={() => setFocused(true)} onBlur={() => { setFocused(false); setPressed(false); }} style={{ ...navLink, position: 'relative', display: 'inline-flex', width: 'fit-content', color, outline: focused ? '2px solid var(--color-brand)' : 'none', outlineOffset: 4, transition: 'color 160ms ease' }}>
        <span>{children}</span>
      </a>
    );
  }
  function ContactLink({ href, children }) {
    ...
    return (
      <a href={href} onMouseEnter={() => setHover(true)} onMouseLeave={() => { setHover(false); setPressed(false); }} onMouseDown={() => setPressed(true)} onMouseUp={() => setPressed(false)} onFocus={() => setFocused(true)} onBlur={() => { setFocused(false); setPressed(false); }} style={{ ...navLink, position: 'relative', display: 'inline-flex', alignItems: 'center', width: 'fit-content', color, outline: focused ? '2px solid var(--color-brand)' : 'none', outlineOffset: 4, transition: 'color 160ms ease' }}>
        <span>{children}</span>
      </a>
    );
  }
    ...
    return (
      <FooterLink
        ...
        onClick={(event) => {
          ...
          if (item.serviceId) window.dispatchEvent(new CustomEvent('np:select-service', { detail: { serviceId: item.serviceId } }));
  ...
  function IconLink({ href, children }) {
    ...
    const [focused, setFocused] = React.useState(false);
    return (
      <a href={href} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)} style={{ display: 'inline-flex', width: 16, height: 16, opacity: hover || focused ? 1 : 0.68, transform: hover || focused ? 'translateY(-1px) scale(1.08)' : 'none', outline: focused ? '2px solid var(--color-brand)' : 'none', outlineOffset: 4, transition: 'opacity 160ms ease, transform 160ms ease' }}>
        {children}
      </a>
    );
  }
  return (
    <footer style={{ padding: isMobile ? 16 : 32, background: 'var(--color-bg)', fontFamily: 'var(--font-body)' }}>
      <div style={{ maxWidth: 1376, margin: '0 auto', boxSizing: 'border-box', padding: isMobile ? 24 : 32, borderRadius: 24, background: '#F2ECE5', display: 'flex', flexDirection: 'column', gap: isMobile ? 40 : 64 }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isNarrow ? '1fr 1fr' : 'minmax(0, 1fr) 214px 214px', gap: isMobile ? 32 : isNarrow ? 48 : 100, alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}><span style={heading}>{t.footer.address}</span><span style={{ ...link, fontWeight: 600 }}>{t.footer.area}<br />{t.footer.mobileService}</span></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}><span style={heading}>{t.footer.phone}</span><ContactLink href="tel:+491605712229">+49 160 5712229</ContactLink></div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}><span style={heading}>{t.footer.email}</span><ContactLink href="mailto:Novopolster@gmail.com">NOVOPOLSTER@GMAIL.COM</ContactLink></div>
          </div>
        ...
        <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', flexDirection: isMobile ? 'column' : 'row', flexWrap: 'wrap', gap: 16, fontFamily: 'var(--font-body)', fontSize: 14, lineHeight: '20px', color: 'rgba(19,20,19,0.64)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
            ...
            <FooterLink href="../../../../impressum.html">Impressum</FooterLink>
            <FooterLink href="../../../../datenschutz.html">Datenschutz</FooterLink>
          </div>
